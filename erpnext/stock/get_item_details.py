...
def get_basic_details(args, item, overwrite_warehouse=True):
	...
	if out.get("weight_per_unit"):
		...
		if interm.get("is_customizable") and interm.get("is_customizable") == 1 and interm.get("customizable_uom") == 'LxW':
			...
			if out['weight_uom'] == 'กก' and out['stock_uom'] == 'กก':
				out["total_weight"] = out.weight_per_unit * out.qty
			else:
...
def apply_price_list(args, as_doc=False):
	...
	if "items" in args:
		...
		for item in item_list:
			...
			logger.info(f'aargs_copy1: {args_copy}')
...
def apply_price_list_on_item(args):
	...
	logger.info(f"iiid args_0 args: {args}")
	logger.info(f"iiid item_doc_0 item_doc: {item_doc}")
	item_details = get_price_list_rate(args, item_doc)
	logger.info(f"iiid get_price_list_rate_1: {item_details}")
	item_details.update(get_pricing_rule_for_item(args))
	logger.info(f"iiid get_price_list_rate_2: {get_pricing_rule_for_item(args)}")
